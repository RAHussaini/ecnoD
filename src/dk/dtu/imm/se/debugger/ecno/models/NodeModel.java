...
public class NodeModel {
	 ...
	 private List<MyConnections> connections;
	    ...
	    public NodeModel() {
	        ...
	        connections = new ArrayList<MyConnections>();
	        MyConnections connect = new MyConnections("1", "1", nodes.get(0),
	                nodes.get(1));
	        connections.add(connect);
	        connect = new MyConnections("2", "2", nodes.get(0), nodes.get(4));
	        connections.add(connect);
	        connect = new MyConnections("3", "3", nodes.get(2), nodes.get(1));
	        connections.add(connect);
	        connect = new MyConnections("4", "3", nodes.get(1), nodes.get(3));
	        connections.add(connect);

	        // Because we are lasy we save the info about the connections in the
	        // nodes

	        for (MyConnections connection : connections) {
